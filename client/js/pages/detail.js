    ...
    async render(container, type, id) {
      ...
      try {
        ...
        btnWL.addEventListener('click', async function () {
          ...
          let res;
          if (isInWatchlist) {
            res = await window.api.watchlist.add({
              tmdb_id: id,
              media_type: type,
            ...
            res = await window.api.watchlist.remove(id, type);
          }
        ...
        console.error('Erreur page detail :', err);
        container.innerHTML = `
          ${window.components.renderHeader()}
          <main class="p-10">
            <p class="text-red-400">Impossible de charger cette page. <a href="/" data-link class="underline">Retour</a></p>
          </main>
        `;
      }
    }
